...
public class HV_Gap_Gl_Exact extends FlowAlgorithm {

	public HV_Gap_Gl_Exact() {
		super("HV_Gap_Gl_Exact");
	}
				...
				if (eligible) {
					...
					while (!relabelQueue.isEmpty()) {
						...
						if (activeHeights[oldHeight].isEmpty() && nonActiveHeights[oldHeight].isEmpty()) {
							...
							//All vertices in the queue were above the gap
							relabelQueue.clear();
						} else if (newHeight >= n) {
							...
							// were higher up, and none of them had excess.
