...
public class HighestVertexGapRelExactDistance extends FlowAlgorithm {

	public HighestVertexGapRelExactDistance() {
		super("HighestVertexGapRelExactDistance");
	}
	...
	protected void initAlgorithm() {
		...
		globalRelabel(); //To start with exact distances
	}
	...
	protected void globalRelabel() {
		...
		//No need to fill in active heights since this is only called at
		//the beginning. Also, no visited since we can use the height.
		b = 0;
		nonActiveHeights[0]++; //t
		q.reset();
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < adj.size(); i++) {
				...
				if (out_v == t || out_v == s) continue;
				out_vertex = vertices.get(out_v);
				if (out_vertex.height > 0) continue; //Has already been visited
